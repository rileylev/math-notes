...
\begin{defn}[Ideal]
  A subset \(I\subseteq R\) is an ideal iff \marginnote{In a rin, you have to distinguish left-ideals and right-ideals}
  \begin{itemize}
...
\end{proof}
Actually, this proof works for any field:
\begin{cor}
  The only ideals of a field are\eit/ \(\set 0\) and the whole field.
\end{cor}
Ideals measure the failure of a ring to be a field.

Ideals also generalize the idea of multiples of a number:
...
\end{proof}
In ``easy'' rings, ideals are just multiples. Unfortunately, life is not so simple.

\begin{theorem}
  \(\Q[x,y]\) has an ideal that's not simply a set of all multiples. It's given by
  \[
    \paren{x,y}
  \]
\end{theorem}
\begin{proof}
  ...
  However, there is no \(\texttt{gcd}(x,y)\) because \(x\) and \(y\) are indefinite variables. The only divisors of \(x\) are \(1\) and \(tx\) where \(t\in\Q\), \(t\neq 0\).
\end{proof}
\begin{theorem}[Unique factorization fails]
  Unique factorization fails in \[\Z\bracket{\sqrt{-5}}.\] There are two distinct ``prime''\marginnote{irreducible \(\neq\) prime, but never mind. They're the same in \(\Z\), but not everywhere.} factorizations of \(6\),
  \[
...
\begin{proof}
  ...
  \[
    N(xy) = N(x)N(y),
  \]
  so necessary condition for \(x\) divides \(z\) is \(N(x)\) divides \(N(z)\). This screens out possible factors, so we can check irreducibility.
  \begin{itemize}
    ...
    \item  \(3\) is irreducible.\\ By the same argument,
          ...
          \]
          so either \(N(x)=1\) or \(N(x)=3\). From \cref{unique-factor-fail-n=1}, we know the only solution for \(N(x)=1\) is \(x=1\). So consider \(N(x)=3\):
          \[
  ...
  \end{itemize}
\end{proof}
Algebraists were not ready to give up unique factorization. So they defined maximal and prime ideals.
...
\begin{defn}[Prime ideal]
  ...
  \]
\end{defn}
But prime ideals need not be maximal.
In fact, the minimal (zero) ideal is often prime
\begin{lemma}
  In a ring with no zero divisors, \(\paren 0\) is prime.
\end{lemma}
\begin{proof}
