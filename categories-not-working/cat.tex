...
\begin{proof}
  ...
  However, there is no \(\texttt{gcd}(x,y)\) because \(x\) and \(y\) are indeterminate: the only divisors of \(x\) (resp. \(y\)) are \(1\) and \(tx\) where \(t\in\Q\), \(t\neq 0\).
\end{proof}
...
\begin{defn}[Prime ideal]
  ...
  \[
    P \ni ab \implies P\ni a \text{ or } P \ni b
  \]
  in analogy with the fact\marginnote{(the \(\divides\) means divides)}
  \[
...
Maximal ideals recover points from the space of functions. But, first, a warmup.
...
\begin{theorem}[Hilbert's Weak Nullenstellensatz]
