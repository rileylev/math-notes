...
\begin{defn}[the diagonal map]  \label{the-diagonal-map}
  ...
  \[
    \begin{tikzcd}[row sep=3em]
      &X\ar[swap,bend right=3em,"1"]{ddl} \ar[bend left=3em,"1"]{ddr}\dar[dashed]{\Delta}\\
...
\begin{defn}[Group object]
  Suppose \(G\in\cat\) and \(\cat\) has all finite products (including a final object, \(F\), the \(0\)-ary product). Suppose there's a multiplication
  \[
    m: G \PI G  \to G,
  \]
  a unit\marginnote{Final object because a unit from an initial object would be very boring. }
  \[
    e : F \to G
  \]
  ...
  \begin{itemize}
    \item \(m\) is associative, \ie,
          \[
          \begin{tikzcd}
            & (G \PI G ) \PI G \ar["m \times 1"]{dr} & \\
            G \PI (G \PI G) \ar["\alpha"]{ur} \dar{1 \times m}  &  & G \PI G\dar{m} \\
            G \PI G \ar["m"]{rr}& & G
          \end{tikzcd}
          \]
          commutes.
    \item \(e\) is a two-sided unit for \(m\), \ie,
          ...
          \begin{tikzcd}[column sep = 4em]
            F \PI G\rar{e \times 1}\ar[swap,"\pi_G"]{dr} & G \PI G\dar{m} & G \PI F \lar[swap]{1 \times m}\ar["\pi_G"]{dl}\\
            & M
          \end{tikzcd}
          \]
          commutes.
...
\]
\ie, they're adjoints:
\begin{defn}[Ten\-sor-hom adjunction]\label{tensor-hom-adj}
...
\begin{defn}
  The \emph{symmetric} tensor algebra \(V^\odot\) of a vector space \(V\) is a ring.
\end{defn}
These are actually the same.
\begin{theorem}
  \[
    R\bracket{X_1,\dots X_n} \cong \gparen{\Span_{\R} \set{X_1\dots X_n}}^\odot
  \]
\end{theorem}
\begin{proof}
  The idea is, in the polynomial ring, we're taking \emph{formal} products of the \(X_i\). We impose no restrictions on \(X_iX_j\) besides associativity, commutativity (the ring laws). There's no further simplification beyond \(X_iX_j\).

  The isomorphism is given by erasing the \(\odot\) symbol:
  \[
    \color{darkgray}
    X_{i_1}\color{black}\odot\color{darkgray} \dots X_{i_k} \mapsto X_{i_1} \dots X_{i_k}
  \]
\end{proof}
...
\[
  0 + 0 = 0
\]
so \(0\) is \emph{closed} under addition,
and
\[
  0x = 0,
\]
\(0\) is \emph{absorbative} under multiplication. Any set that can be collapsed to zero must have these properties as well. It turns out they are sufficient.
...
\end{theorem}
\begin{proof}\phantom{M}
  \begin{itemize}
    \item If field, the only ideals are \(0\) and \(R\).\\
          Take an ideal \(I\neq 0\). Pick a nonzero \(x\in I\). If \(x\) has an inverse, then by absorbativity
          \[
          xx^{-1} = 1 \in I.
          \]
          But then, by absorbativity, \(I=R\).
    \item If the only ideals are \(R\) and \(0\), it's a field.  \\
          Pick a nonzero \(x\in R\). Then \(\paren{x}\), the set of multiples of \(x\), is an ideal. By hypothesis, it contains \(1\)---by hypothesis, \(1\) is a multiple of \(x\). Therefore, there is some \(y\) such that\revmarginnote{In a rin, we need an inverse on both sides. }
          \[
          xy =1.
          \]
  \end{itemize}
...
Algebraic geometry \emph{was} the study of systems of polynomial equations, until the search for a cohomology theory for finite fields ({\footnotesize\url{https://en.wikipedia.org/wiki/Weil_conjectures}}), drove algebraic geometers to madness. But, if you look closely, you can still see the polynomial equations, peering out from behind the sheaves, schemes, and ideals.
...
\begin{theorem}[Hilbert's Weak Nullstellensatz]\label{nullstellensatz}
...
This hints at a \emph{geometric interpretation of ideals}. Suppose \(I\) is an ideal, not maximal, over \(\complex[x_1\dots x_n]\). Then \(I\) can be \emph{refined} to some maximal ideal:
\[
  I\subseteq (x_1-a_1,\dots x_n-a_n).
\]
Geometrically, this means the subset of \(\complex^n\) associated to \(I\) \emph{contains} the point \((a_1,\dots a_n)\).
...
\begin{proof}
  ...
  \]
\end{proof}
Actually, the \nameref{remainder-root} lets us skip all the work.
\begin{proof}
  By \namecref{remainder-root},
  \[
    \gparen{y-x^2} \equiv y-a^2 \pmod {x-a}
  \]
  therefore
  \[
    y-x^2 = q(x,y) \paren{x-a} + \paren{y-a^2}
  \]
  so
  \[
    y-x^2 \in \paren{x-a,y-a^2}.
  \]
\end{proof}
...
\begin{itemize}
  ...
  \item Riehl's \book{Category theory in context} \cite{cat-context}. I found this from a thread on stack-exchange:
        {\tiny\url{https://math.stackexchange.com/questions/370/good-books-and-lecture-notes-about-category-theory}}. So far I've only used it to find some examples, but it seems solid! Here's some good quotes from the preface:
        \begin{itemize}
