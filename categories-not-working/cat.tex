...
\[
  \set{\frac{p(x)}{(x-a)^m} + \frac{q(x)}{(x-b)^n}}
\]
Shrinking the domain grows the set of functions, which is backwards from \(\setcat\), \(\topcat\), \(\smooth\).
